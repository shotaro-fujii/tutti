...
//--------------------------------------------------------------------------------------------
    ...
    func button_animate(_ sender:UITapGestureRecognizer){
        //tagの番号を調べる
    ...
    */
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        // Labelを作成
        let label: UILabel = UILabel(frame:CGRect(x:0,y:0,width:cellSize,height:30))
        label.text = (totalBox[indexPath.row] as AnyObject).value(forKey: "title") as? String
        label.textColor = UIColor.black
        //label.shadowColor = UIColor.gray
        label.textAlignment = NSTextAlignment.center
        label.layer.position = CGPoint(x:label.bounds.width/2,y:cellSize-15)
        //print(cellSize)
