...
class News4ViewController: UIViewController,UIWebViewDelegate,XMLParserDelegate , UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    
    var urlArray = [String]()
    
    var tagArray = [String]()
    var tag_outputArray = [String]()
    ...
    var myTagString = NSMutableString()
    var mystring = String()
    ...
    override func viewDidLoad() {
        ...
        //let url:String = "http://192.168.11.34/?feed=rss2&cat=2"
        //let url:String = "http://192.168.11.34/?feed=rss2&cat=4"
        //let url:String = "http://192.168.11.34/?feed=rss2&cat=3"
        let url:String = "http://localhost:8888/?feed=rss2&cat=6"
    ...
    func clip(){
        ...
        myTagString.append(mytag)
        myElements.setObject(myTagString, forKey: "tag" as NSCopying)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let tagString = (totalBox[indexPath.row] as AnyObject).value(forKey: "tag") as? String?
        
        mytag = tagString as! String
        //mytag = "マイタグ"
        print("mytag:", mytag)
    ...
    func parser(_ parser: XMLParser, foundCharacters string: String) {
        ...
        if element == "title"{
            ...
            if tagArray.last! == "食"{
                print("「食」カテゴリーです")
            } else if tagArray.last! == "運動"{
                print("「運動」カテゴリーです")
            } else if tagArray.last! == "癒し"{
                print("「癒し」カテゴリーです")
            } else if tagArray.last! == "学び"{
                print("「学び」カテゴリーです")
            } else{
                tag_outputArray.append(String(tagArray.last!))
            }
            print("アプトプット:", tag_outputArray)
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        ...
        if elementName == "item"{
            ...
            print("アプトプット:", tag_outputArray[1])

            elements.setObject(tag_outputArray[1], forKey: "tag" as NSCopying)
            
            
            //totalBoxの中にelementsを入れる
            totalBox.add(elements)
            print("total_box:", totalBox)
